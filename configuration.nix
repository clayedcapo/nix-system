...
# | - hostname: Your hostname (from specialArgs)
{
  ...
  documentation = {
    ...
    man.cache.enable = true; # Faster apropos/whatis lookups
  ...
  services.qbittorrent = {
    ...
    webuiPort = 7780;
    # Open firewall for BitTorrent traffic
    openFirewall = true;
    # Run as your user so downloads can go anywhere in ~/
    user = username;
    group = "users";
  };

  # -------------------------------------------------------------------------
  # nix-ld (FHS binary compatibility)
  # -------------------------------------------------------------------------
  # Provides /lib/ld-linux-x86-64.so.2 so dynamically linked binaries
  # (e.g. compiled by Jai) run without manual patchelf
